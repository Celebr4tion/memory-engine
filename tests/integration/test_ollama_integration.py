...
class TestOllamaIntegration:
    ...
    async def test_model_info_retrieval(self, ollama_provider):
        ...
        ), f"Expected some model info fields, got: {list(info.keys())}"
    ...
    async def test_available_models_listing(self, ollama_provider):
        ...
        assert (
            ollama_provider.model_name in models
        ), f"Configured model should be in available models: {models}"
    ...
    async def test_dimension_detection(self, ollama_provider):
        ...
        assert (
            detected_dim == actual_dim
        ), f"Detected dimension {detected_dim} should match actual {actual_dim}"
    ...
    async def test_concurrent_requests(self, ollama_provider):
        ...
        tasks = [ollama_provider.generate_embedding(text) for text in texts]
