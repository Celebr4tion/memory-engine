...
from memory_core.quality.quality_assessment import (
    QualityAssessmentEngine, QualityDimension, QualityLevel
)
from memory_core.quality.cross_validation import (
    CrossValidationEngine, ValidationStatus, ValidationConfidence
)
from memory_core.quality.source_reliability import (
    SourceReliabilityEngine, ReliabilityLevel, SourceType
)
from memory_core.quality.gap_detection import (
    KnowledgeGapDetector, GapType, GapSeverity
)
from memory_core.quality.contradiction_resolution import (
    ContradictionResolver, ResolutionStrategy, ContradictionSeverity
)
from memory_core.quality.quality_enhancement_engine import (
    KnowledgeQualityEnhancementEngine, QualityScore, EnhancementAction
)
...
class TestCrossValidationEngine:
    ...
    def test_contradiction_detection(self, mock_query_engine, sample_nodes):
        ...
        if contradictions:
            assert any("1989" in str(c.conflicting_claims) or "1991" in str(c.conflicting_claims) 
...
class TestIntegrationScenarios:
    ...
    def test_full_quality_enhancement_workflow(self, mock_query_engine, sample_nodes):
        ...
        low_quality_nodes = [score for score in report.quality_scores 
                           if score.overall_score < 0.6]
        
        if low_quality_nodes:
            # Should have recommendations for low quality nodes
            relevant_recommendations = [rec for rec in report.enhancement_recommendations 
