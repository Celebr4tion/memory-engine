...
class TestAutoRelationshipByTags(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node_data = {
            "node1": {"tags": "ai,ml,neural networks"},
            "node2": {"tags": "ai,deep learning,computer vision"},
            "node3": {"tags": "blockchain,crypto,distributed systems"},
            "node4": {"content": "Node without tags"},  # No tags
            "node5": {"tags": "ai,ml"},  # High overlap with node1
        }
    ...
    def test_detects_tag_based_relationships(self):
        ...
        expected_pairs = [("node1", "node2"), ("node1", "node5"), ("node2", "node5")]
...
class TestCreateDomainRelationships(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create test node data
        self.node_data = {
            "node1": {"extra_metadata": json.dumps({"domain": "computer science"})},
            "node2": {"extra_metadata": json.dumps({"domain": "computer science"})},
            "node3": {"extra_metadata": json.dumps({"domain": "physics"})},
            "node4": {"extra_metadata": json.dumps({"language": "english"})},  # No domain
            "node5": {"extra_metadata": json.dumps({"domain": "physics"})},
        }
    ...
    def test_detects_domain_based_relationships(self):
        ...
        for call in call_args_list:
            ...
            edge_details.append((tuple(sorted([from_id, to_id])), relation_type))

        # Expected edge details
        expected_edges = [(("node1", "node2"), "SAME_DOMAIN"), (("node3", "node5"), "SAME_DOMAIN")]
...
class TestSemanticRelationships(unittest.TestCase):
    ...
    def test_suggest_semantic_relationships(self):
        ...
        suggestions = suggest_semantic_relationships(
            node_ids, self.mock_embedding_manager, self.mock_storage
        )
    ...
    def test_returns_empty_list_when_no_embedding_manager(self):
        ...
        node_ids = ["node1", "node2"]
        suggestions = suggest_semantic_relationships(
            node_ids, embedding_manager=None, storage=self.mock_storage
        )
...
class TestAnalyzeAndCreateRelationships(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.auto_tag_patcher = patch(
            "memory_core.ingestion.relationship_extractor.auto_relationship_by_tags"
        )
        ...
        self.domain_patcher = patch(
            "memory_core.ingestion.relationship_extractor.create_domain_relationships"
        )
        self.mock_domain = self.domain_patcher.start()
        self.mock_domain.return_value = ["domain_edge1"]

        self.semantic_patcher = patch(
            "memory_core.ingestion.relationship_extractor.suggest_semantic_relationships"
        )
    ...
    def test_calls_all_relationship_methods(self):
        ...
        results = analyze_and_create_relationships(
            node_ids, self.mock_storage, self.mock_embedding_manager
        )
    ...
    def test_works_without_embedding_manager(self):
        ...
        node_ids = ["node1", "node2"]
        results = analyze_and_create_relationships(
            node_ids, self.mock_storage, embedding_manager=None
        )
