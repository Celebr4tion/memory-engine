...
class MockAnthropicResponse:
    ...
    def __init__(self, content: str, model: str = "claude-3-5-sonnet-20241022", 
                 stop_reason: str = "end_turn", usage: Dict = None):
...
def mock_anthropic_available():
    ...
    with patch('memory_core.llm.providers.anthropic.anthropic_provider.ANTHROPIC_AVAILABLE', True):
        with patch('memory_core.llm.providers.anthropic.anthropic_provider.AsyncAnthropic') as mock_client:
...
class TestAnthropicLLMProviderInitialization:
    ...
    def test_anthropic_not_available(self):
        ...
        with patch('memory_core.llm.providers.anthropic.anthropic_provider.ANTHROPIC_AVAILABLE', False):
...
class TestAnthropicLLMProviderCompletion:
    ...
    async def test_structured_output_request(self, provider):
        ...
        await provider.generate_completion(
            ...
            task_type=LLMTask.KNOWLEDGE_EXTRACTION
        )
...
class TestAnthropicLLMProviderTasks:
    ...
    async def test_validate_content_success(self, provider):
        ...
        validation_data = {
            "valid": True,
            "criteria_results": {
                "Has proper grammar": True,
                "Contains factual information": True
            },
            "overall_score": 0.95
        }
...
class TestAnthropicLLMProviderErrorHandling:
    ...
    async def test_anthropic_rate_limit_error(self, provider):
        ...
        with patch('memory_core.llm.providers.anthropic.anthropic_provider.anthropic') as mock_anthropic:
