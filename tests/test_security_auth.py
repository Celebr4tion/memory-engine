...
from memory_core.security.auth import AuthManager, User, UserSession, UserStatus
...
class TestUserSession:
    ...
    def test_session_refresh(self):
        ...
        session = UserSession(
            session_id="session-123", user_id="user-123", username="testuser", roles=set()
        )
...
class TestAuthManager:
    ...
    def test_create_user_success(self):
        ...
        user = self.auth_manager.create_user(
            username="testuser", email="test@example.com", password="ValidPass123!", roles={"admin"}
        )
    ...
    def test_create_user_duplicate_username(self):
        ...
        self.auth_manager.create_user(
            username="testuser", email="test1@example.com", password="ValidPass123!"
        )

        with pytest.raises(ValueError, match="Username.*already exists"):
            self.auth_manager.create_user(
                username="testuser", email="test2@example.com", password="ValidPass123!"
            )

    def test_create_user_duplicate_email(self):
        """Test creating user with duplicate email."""
        self.auth_manager.create_user(
            username="testuser1", email="test@example.com", password="ValidPass123!"
        )

        with pytest.raises(ValueError, match="Email.*already exists"):
            self.auth_manager.create_user(
                username="testuser2", email="test@example.com", password="ValidPass123!"
            )
    ...
    def test_session_management(self):
        ...
        session = self.auth_manager.create_session(
            user, ip_address="192.168.1.1", user_agent="Test Agent"
        )
    ...
    def test_user_management_operations(self):
        ...
        # Update user
        result = self.auth_manager.update_user(user.user_id, status=UserStatus.SUSPENDED)
        assert result is True
    ...
    def test_session_list_and_cleanup(self):
        ...
        user2 = self.auth_manager.create_user(
            username="user2", email="user2@example.com", password="ValidPass123!"
        )
