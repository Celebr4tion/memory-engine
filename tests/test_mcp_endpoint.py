...
class TestMemoryEngineMCP(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.vector_store_patcher = patch('memory_core.mcp_integration.mcp_endpoint.VectorStoreMilvus')
        ...
        self.embedding_manager_patcher = patch('memory_core.mcp_integration.mcp_endpoint.EmbeddingManager')
        ...
        self.extract_knowledge_patcher = patch('memory_core.mcp_integration.mcp_endpoint.extract_knowledge_units')
        ...
        self.process_units_patcher = patch('memory_core.mcp_integration.mcp_endpoint.process_extracted_units')
        ...
        self.create_relationships_patcher = patch('memory_core.mcp_integration.mcp_endpoint.analyze_and_create_relationships')
    ...
    def test_ingest_raw_text(self):
        ...
        result = self.mcp.ingest_raw_text(
            "This is some test text to ingest.", 
    ...
    def test_search_text_with_no_embedding_manager(self):
        ...
        with patch('memory_core.mcp_integration.mcp_endpoint.VectorStoreMilvus') as mock_vector_store_class:
    ...
    def test_get_node_details(self):
        ...
        self.mock_engine.get_outgoing_relationships.return_value = [
            MagicMock(edge_id='edge1', to_id='related1', relation_type='RELATED', confidence_score=0.7)
        ]
        
        self.mock_engine.get_incoming_relationships.return_value = [
            MagicMock(edge_id='edge2', from_id='related2', relation_type='SEMANTICALLY_SIMILAR', confidence_score=0.8)
        ]
    ...
    def test_update_node_rating(self):
        ...
        with patch.object(self.mcp, 'get_node_details') as mock_get_details:
            ...
            # Call the method
            result = self.mcp.update_node_rating('test123', {
                'confirmation': 1,
                'richness': 0.5
            })
    ...
    def test_execute_mcp_command_ingest_text(self):
        ...
        with patch.object(self.mcp, 'ingest_raw_text') as mock_ingest:
            mock_ingest.return_value = {'status': 'success', 'created_or_merged_node_ids': ['node1']}
