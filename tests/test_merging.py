...
class TestMergingFunctions(unittest.TestCase):
    ...
    def test_merge_node_data(self):
        ...
        # Test merging tags
        existing = {"content": "Test content", "tags": "ai,machine learning"}

        new = {"content": "New content", "tags": "ai,neural networks"}
        ...
        existing = {
            "content": "Test content",
            "extra_metadata": json.dumps({"domain": "computer science", "importance": 0.7}),
        }

        new = {
            "content": "New content",
            "extra_metadata": json.dumps(
                {"domain": "artificial intelligence", "language": "english"}
            ),
        }
        ...
        new = {"content": "New content", "source_details": "Type: article; Ref: Example et al."}

        merged = _merge_node_data(existing, new)

        # Source details should be combined
        expected_details = set(
            ["Type: webpage", "URL: https://example.com", "Type: article", "Ref: Example et al."]
        )
...
class TestMergingIntegration:
    ...
    def setup_method(self):
        ...
        try:
            ...
            if self.embedding_manager is None or not hasattr(
    ...
    def test_merge_similar_nodes(self):
        ...
        if (
            ...
            or not self.embedding_manager.vector_store.connected
        ):
        ...
        try:
            ...
            if embedding_manager:  # Merged case
                ...
                expected_tags_merged = sorted(
                    list(expected_tags_node1.union(set(self.node2_data.get("tags", "").split(","))))
                )
                actual_tags = sorted(retrieved_node1.get("tags", "").split(","))
                assert (
                    actual_tags == expected_tags_merged
                ), f"Expected {expected_tags_merged}, got {actual_tags}"
            else:  # Separate case
