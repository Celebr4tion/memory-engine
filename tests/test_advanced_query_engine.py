...
from memory_core.query import (
    AdvancedQueryEngine, QueryRequest, QueryResponse, QueryResult,
    QueryType, FilterCondition, SortCriteria, SortOrder, AggregationRequest, AggregationType
)
...
class TestAdvancedQueryEngine:
    ...
    def test_explanation(self, query_engine):
        ...
        request = QueryRequest(
            query="test query with explanation",
            explain=True
        )
    ...
    def test_performance_stats(self, query_engine):
        ...
        assert updated_stats['query_engine']['total_queries'] > initial_stats['query_engine']['total_queries']
    ...
    def sample_results(self):
        ...
        return [
            QueryResult(
                ...
                metadata={"rating_richness": 0.9, "rating_truthfulness": 0.8, "creation_timestamp": time.time()}
            ),
            QueryResult(
                node_id="node2",
                content="Medium quality content", 
                metadata={"rating_richness": 0.6, "rating_truthfulness": 0.7, "creation_timestamp": time.time() - 86400}
            ),
            QueryResult(
                node_id="node3",
                content="Lower quality content",
                metadata={"rating_richness": 0.4, "rating_truthfulness": 0.5, "creation_timestamp": time.time() - 172800}
            )
        ]
...
class TestQueryOptimizer:
    ...
    def test_basic_optimization(self, query_optimizer):
        ...
        optimized_request, execution_plan, explanation_steps = query_optimizer.optimize_query(request)
