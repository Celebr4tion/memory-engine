...
from memory_core.storage.factory import (
    create_storage,
...
class TestStorageBackends(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_edge_data = {"timestamp": time.time(), "confidence_score": 0.85, "version": 1}
    ...
    def test_storage_factory(self):
        ...
        if SQLITE_AVAILABLE:
            sqlite_storage = create_storage(
                backend_type="sqlite", config_override={"database_path": str(self.test_db_path)}
            )
    ...
    def test_json_file_storage_specific_features(self):
        ...
        storage = JsonFileStorage(
            directory=str(Path(self.test_dir) / "json_specific"), pretty_print=True
        )
    ...
    def test_knowledge_node_operations_across_backends(self):
        ...
        backends = [JsonFileStorage(directory=str(Path(self.test_dir) / "json_kn"))]
        if SQLITE_AVAILABLE:
    ...
    def test_relationship_operations_across_backends(self):
        ...
        backends = [JsonFileStorage(directory=str(Path(self.test_dir) / "json_rel"))]
        if SQLITE_AVAILABLE:
    ...
    def _test_storage_basic_operations(self, storage):
        ...
        edge_id = self._run_async_sync(
            storage.create_edge(node_id, node2_id, "test_relation", self.test_edge_data)
        )
