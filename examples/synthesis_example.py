...
def demonstrate_question_answering():
    ...
    context = QuestionContext(
        domain="healthcare",
        entities=["AI", "artificial intelligence", "medical diagnostics"]
    )
...
def demonstrate_comprehensive_synthesis():
    ...
    if result.insights:
        ...
        print("-"*30)
        total_insights = (len(result.insights.patterns) + 
    ...
    if result.perspectives:
        ...
        print(f"Consensus Level: {result.perspectives.overall_consensus.value.replace('_', ' ').title()}")
