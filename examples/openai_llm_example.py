...
async def main():
    ...
    try:
        ...
        for i, rel in enumerate(relationships[:3], 1):  # Show first 3 relationships
            print(f"   Relationship {i}: {rel.get('source')} --[{rel.get('relationship_type')}]--> {rel.get('target')}")
        ...
        messages = [
            Message(MessageRole.SYSTEM, "You are a helpful AI assistant specializing in technology."),
            Message(MessageRole.USER, "What are the main differences between supervised and unsupervised learning?"),
        ]
        
        chat_response = await provider.generate_chat_completion(messages, LLMTask.GENERAL_COMPLETION)
