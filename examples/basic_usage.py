...
def example_3_embeddings_and_search(
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                if "rating_truthfulness" in result["updates"]:
                    print(
                        f"      📊 Truthfulness: {old_truthfulness:.2f} → {updated_node.rating_truthfulness:.2f}"
                    )
                if "rating_richness" in result["updates"]:
                    print(
                        f"      📊 Richness: {old_richness:.2f} → {updated_node.rating_richness:.2f}"
                    )
                if "rating_stability" in result["updates"]:
                    print(
                        f"      📊 Stability: {old_stability:.2f} → {updated_node.rating_stability:.2f}"
                    )
            else:
