...
async def compare_backends():
    ...
    for backend, results in performance_results.items():
        if "error" in results:
            ...
            print(
                f"{backend:12} | Creation: {results['creation_time']:.3f}s | "
...
async def migration_example():
    ...
    json_storage = create_storage(
        backend_type="json_file", config_override={"directory": "./migration_source"}
    )
    ...
    if is_backend_available("sqlite"):
        ...
        sqlite_storage = create_storage(
            backend_type="sqlite", config_override={"database_path": "./migration_target.db"}
        )
...
async def main():
    ...
    # JSON File backend (always available)
    await demonstrate_backend("json_file", config_override={"directory": "./demo_json_storage"})

    # SQLite backend (if aiosqlite is installed)
    if is_backend_available("sqlite"):
        await demonstrate_backend("sqlite", config_override={"database_path": "./demo_storage.db"})
    else:
