...
async def chat_completion_example(provider: AnthropicLLMProvider):
    ...
    try:
        ...
        messages = [
            Message(role=MessageRole.SYSTEM, content="You are a helpful AI assistant specializing in technology."),
            Message(role=MessageRole.USER, content="What is machine learning?"),
            Message(role=MessageRole.ASSISTANT, content="Machine learning is a subset of artificial intelligence that enables computers to learn and make decisions from data without being explicitly programmed for every task."),
            Message(role=MessageRole.USER, content="Can you give me an example of how it's used in practice?")
        ]
    ...
    try:
        ...
        for query in queries:
            ...
            parsed = await provider.parse_natural_language_query(
                query=query,
                context="Knowledge graph containing programming and AI concepts"
            )
