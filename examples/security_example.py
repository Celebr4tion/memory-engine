...
async def main():
    ...
    print(f"   ✓ Editor can access confidential node (after rule): {editor_can_access_confidential_now}")
    ...
    encrypted_api_key = encryption_manager.encrypt_string(
        api_key, EncryptionScope.API_KEYS
    )
    ...
    print(f"   ✓ Security summary: {security_summary['total_events']} events, {security_summary['unique_users']} users")
