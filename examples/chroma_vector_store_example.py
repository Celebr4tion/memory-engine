...
async def main():
    ...
    try:
        ...
        sample_vectors = [
            np.random.rand(config['dimension']).astype(np.float32) for _ in range(10)
        ]
        sample_ids = [f"doc_{i}" for i in range(10)]
        sample_metadata = [
            {"category": "science" if i % 2 == 0 else "technology", "index": i}
            for i in range(10)
        ]
        ...
        update_success = await vector_store.update_vector(
            vector_id="doc_0",
            metadata={"category": "updated", "status": "modified"}
        )
