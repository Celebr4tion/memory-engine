...
async def main():
    ...
    query_engine = AdvancedQueryEngine(
        graph_adapter=graph_adapter, embedding_manager=embedding_manager, rating_storage=None
    )
    ...
    semantic_request = QueryRequest(
        ...
        sort_by=[SortCriteria(field="relevance_score", order=SortOrder.DESCENDING)],
        limit=5,
    )
    ...
    agg_request = QueryRequest(
        ...
        aggregations=[
            ...
            AggregationRequest(type=AggregationType.AVERAGE, field="metadata.rating"),
        ],
    )
    ...
    filter_request = QueryRequest(
        ...
        filters=[
            ...
            FilterCondition(
                field="metadata.tags", operator="in", value=["programming", "tutorial"]
            ),
    ...
    cache_request = QueryRequest(query="cached query example", use_cache=True, cache_ttl=3600)
...
class MockGraphAdapter:
    ...
    def get_relationships_for_node(self, node_id, max_depth=3):
        return [{"type": "related_to", "target": f"related_{node_id}", "weight": 0.8}]
