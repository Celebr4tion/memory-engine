...
def example_3_process_and_store_knowledge(
    engine: KnowledgeEngine, embedding_manager: EmbeddingManager
):
...
def example_4_automatic_relationships(
    engine: KnowledgeEngine, embedding_manager: EmbeddingManager, node_ids: List[str]
):
    ...
    try:
        ...
        relationships = analyze_and_create_relationships(
            node_ids, engine.storage, embedding_manager
        )
...
def example_6_batch_processing(engine: KnowledgeEngine, embedding_manager: EmbeddingManager):
    ...
    all_node_ids = []
    processing_stats = {"total_units": 0, "total_nodes": 0, "processing_time": 0}
    ...
    try:
        relationships = analyze_and_create_relationships(
            all_node_ids, engine.storage, embedding_manager
        )
    ...
    print(
        f"   📈 Units per second: {processing_stats['total_units'] / processing_stats['processing_time']:.2f}"
    )
