...
class QueryValidator:
    ...
    def __init__(self):
        self.supported_fields = {
            'nodes': ['id', 'content', 'source', 'timestamp', 'rating_truthfulness', 
                     'rating_importance', 'rating_novelty', 'tags', 'metadata'],
    ...
    def validate_query(self, query_spec: QuerySpec) -> List[str]:
        ...
        for filter_condition in query_spec.filters:
            ...
            if (query_type_str in self.supported_fields and 
                filter_condition.field not in self.supported_fields[query_type_str]):
                errors.append(f"Invalid field '{filter_condition.field}' for query type '{query_type_str}'")
            ...
            if query_type_str in self.supported_fields:
                for field_name in query_spec.projection.fields:
                    if field_name not in self.supported_fields[query_type_str]:
                        errors.append(f"Invalid projection field '{field_name}' for query type '{query_type_str}'")
...
class GraphQLQueryProcessor(QueryProcessor):
    ...
    async def _execute_graph_query(self, query_spec: QuerySpec) -> Dict[str, Any]:
        ...
        nodes_result = await self._execute_node_query(
            QuerySpec(QueryType.NODES, query_spec.filters, query_spec.projection,
                     [], query_spec.sorts, query_spec.limit, query_spec.offset)
        )
        
        relationships_result = await self._execute_relationship_query(
            QuerySpec(QueryType.RELATIONSHIPS, query_spec.filters, query_spec.projection,
                     [], query_spec.sorts, query_spec.limit, query_spec.offset)
        )
    ...
    def _apply_filters(self, data: List[Dict[str, Any]], 
                      filters: List[FilterCondition]) -> List[Dict[str, Any]]:
    ...
    def _item_matches_filters(self, item: Dict[str, Any], 
    ...
    def _evaluate_filter(self, item: Dict[str, Any], 
                        filter_cond: FilterCondition) -> bool:
        ...
        if filter_cond.operator == FilterOperator.EQ:
            ...
            return (isinstance(field_value, str) and isinstance(filter_value, str) and 
                   filter_value.lower() in field_value.lower())
        elif filter_cond.operator == FilterOperator.STARTS_WITH:
            return (isinstance(field_value, str) and isinstance(filter_value, str) and 
                   field_value.lower().startswith(filter_value.lower()))
        elif filter_cond.operator == FilterOperator.ENDS_WITH:
            return (isinstance(field_value, str) and isinstance(filter_value, str) and 
                   field_value.lower().endswith(filter_value.lower()))
        elif filter_cond.operator == FilterOperator.REGEX:
            return (isinstance(field_value, str) and isinstance(filter_value, str) and 
                   re.search(filter_value, field_value) is not None)
    ...
    def _apply_sorting(self, data: List[Dict[str, Any]], 
                      sorts: List[QuerySort]) -> List[Dict[str, Any]]:
        ...
        def sort_key(item):
            ...
            for sort_spec in sorts:
                ...
                key_values.append(value if sort_spec.ascending else 
                                 self._reverse_sort_value(value))
    ...
    def _apply_pagination(self, data: List[Dict[str, Any]], 
    ...
    def _apply_projection(self, data: List[Dict[str, Any]], 
                         projection: Optional[QueryProjection]) -> List[Dict[str, Any]]:
    ...
    def _apply_aggregation(self, data: List[Dict[str, Any]], 
                          aggregation: QueryAggregation) -> Any:
        """Apply aggregation to data."""
        field_values = [item.get(aggregation.field) for item in data 
