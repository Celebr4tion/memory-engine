...
class KnowledgeQueryOperation(StreamingOperation):
    ...
    def __init__(self, operation_id: str, parameters: Dict[str, Any], knowledge_engine: Any):
    ...
    async def execute(self) -> AsyncGenerator[StreamingResult, None]:
        ...
        try:
            ...
            self._emit_progress(
                ProgressType.COUNT, 0, total_results, f"Found {total_results} results, streaming..."
            )
            ...
            yield StreamingResult(
                operation_id=self.operation_id, state=StreamingState.FAILED, data=None, error=str(e)
            )
...
class BulkImportOperation(StreamingOperation):
    ...
    async def execute(self) -> AsyncGenerator[StreamingResult, None]:
        ...
        try:
            ...
            self._emit_progress(
                ProgressType.COUNT, 0, total_items, f"Starting bulk import of {total_items} items"
            )
            ...
            if not self.is_cancelled():
                self.state = StreamingState.COMPLETED
                self._emit_progress(
                    ProgressType.COUNT, total_items, total_items, "Bulk import completed"
                )
...
class MCPStreaming:
    ...
    async def start_knowledge_query(
        self,
    ...
    async def stream_operation(self, operation_id: str) -> AsyncGenerator[StreamingResult, None]:
        ...
        try:
            ...
            error_result = StreamingResult(
                operation_id=operation_id, state=StreamingState.FAILED, data=None, error=str(e)
            )
    ...
    def get_operation_status(self, operation_id: str) -> Optional[StreamingResult]:
        ...
        if operation_id in self.active_operations:
            ...
            return StreamingResult(operation_id=operation_id, state=op.state, data={"active": True})
...
class EnhancedMCPServer:
    ...
    async def _handle_stream_import(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            operation_id = await self.streaming.start_bulk_import(parameters, self.knowledge_engine)
    ...
    async def _handle_list_operations(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"success": True, "active_operations": active_ops, "count": len(active_ops)}
