...
class SecurityMiddleware:
    ...
    def is_public_endpoint(self) -> bool:
        ...
        endpoint = request.endpoint
        return endpoint in public_endpoints or (
            endpoint and any(endpoint.startswith(prefix) for prefix in ["public_", "api.public_"])
        )
    ...
    def is_rate_limited(self) -> bool:
        ...
        if client_id in self._rate_limit_storage:
            ...
            self._rate_limit_storage[client_id] = [
                req_time
                for req_time in self._rate_limit_storage[client_id]
        ...
        def decorated_function(*args, **kwargs):
            ...
            if resource_id_param:
                ...
                if resource_id is None:
                    resource_id = (
                        request.view_args.get(resource_id_param) if request.view_args else None
                    )
...
def require_privacy_level(
    min_privacy_level: PrivacyLevel, resource_type: str, resource_id_param: str
) -> Callable:
    """
            ...
            try:
                ...
                if hasattr(g, "audit_logger") and g.audit_logger:
                    ...
                    g.audit_logger.log_event(
                        ...
                        session_id=(
                            g.security.session.session_id
                            if hasattr(g, "security") and g.security.session
                            else None
                        ),
                        success=success,
                        error_message=error_message,
        ...
        def decorated_function(*args, **kwargs):
            ...
            if user_key in call_storage:
                call_storage[user_key] = [
                    call_time for call_time in call_storage[user_key] if call_time > window_start
                ]
