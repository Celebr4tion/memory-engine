...
class KnowledgeAccessControl:
    ...
    def get_resource_metadata(self, resource_type: str, resource_id: str) -> Optional[KnowledgeAccessMetadata]:
    ...
    def check_access(
        ...
        if metadata.owner_id == user_id:
            ...
            result = self._evaluate_access(
                user_id, user_roles, metadata, resource_type, resource_id, permission_type, organization_id
            )
    ...
    def _evaluate_access(
        ...
        if metadata.privacy_level == PrivacyLevel.RESTRICTED:
            ...
            return self._check_explicit_access(user_id, user_roles, resource_type, resource_id, permission_type)
        
        if metadata.privacy_level == PrivacyLevel.CONFIDENTIAL:
            # Confidential resources require appropriate role or explicit access
            has_role_access = self.rbac_manager.check_permission(user_roles, permission_type, resource_id)
            has_explicit_access = self._check_explicit_access(user_id, user_roles, resource_type, resource_id, permission_type)
        ...
        if metadata.privacy_level == PrivacyLevel.INTERNAL:
            ...
            if organization_id and metadata.organization_id and organization_id == metadata.organization_id:
                return True
    ...
    def _check_explicit_access(
        ...
        resource_rules = [
            rule for rule in self._access_rules.values()
    ...
    def _evaluate_conditions(self, conditions: Dict[str, any], user_id: str, user_roles: Set[str]) -> bool:
    ...
    def _clear_resource_cache(self, resource_type: str, resource_id: str) -> None:
        ...
        for user_cache in self._access_cache.values():
            keys_to_remove = [
                key for key in user_cache.keys()
    ...
    def get_accessible_resources(
        ...
        for metadata_key, metadata in self._resource_metadata.items():
            ...
            if self.check_access(user_id, user_roles, resource_type, resource_id, permission_type, organization_id):
                accessible.append(resource_id)
        
        return accessible
    
    def audit_access_attempts(
        self,
        user_id: str,
        days_back: int = 30
    ) -> List[Dict[str, any]]:
