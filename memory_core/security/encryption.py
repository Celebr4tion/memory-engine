...
class EncryptionManager:
    ...
    def encrypt(
        ...
        if key.algorithm == EncryptionType.FERNET:
            ...
            max_chunk_size = (
                key.algorithm == EncryptionType.RSA_2048 and 190
            ) or 446  # Conservative sizes
    ...
    def rotate_key(self, scope: EncryptionScope) -> EncryptionKey:
        ...
        logger.info(
            f"Rotated key for scope {scope.value}: {current_key.key_id if current_key else 'none'} -> {new_key.key_id}"
        )
        return new_key
