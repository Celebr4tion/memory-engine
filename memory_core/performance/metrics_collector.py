    ...
    def record(self, value: Union[int, float], tags: Dict[str, str] = None):
        ...
        with self._lock:
            metric_value = MetricValue(
                value=value, timestamp=time.time(), tags={**self.tags, **(tags or {})}
            )
...
class MetricsCollector:
    ...
    def histogram(
        self, name: str, buckets: List[float] = None, tags: Dict[str, str] = None
    ) -> Histogram:
    ...
    def rate(self, name: str, window_size: int = 60, tags: Dict[str, str] = None) -> Rate:
...
class PerformanceMonitor:
    ...
    def track_request(self, component: str, operation: str):
        ...
        self.metrics.counter(f"{component}.requests.total", {"operation": operation}).increment()

        # Mark request rate
        self.metrics.rate(f"{component}.requests.rate", tags={"operation": operation}).mark()

        # Return timer context for duration tracking
        return self.metrics.timer(f"{component}.requests.duration", {"operation": operation}).time()

    def track_error(self, component: str, operation: str, error_type: str):
        """Track an error."""
        self.metrics.counter(
            f"{component}.errors.total", {"operation": operation, "error_type": error_type}
        ).increment()

    def track_resource_usage(self, component: str, resource: str, value: Union[int, float]):
        """Track resource usage."""
        self.metrics.gauge(f"{component}.resources.{resource}", {"component": component}).set(value)

    def track_cache_hit(self, cache_name: str):
        """Track cache hit."""
        self.metrics.counter(f"cache.hits.total", {"cache": cache_name}).increment()

    def track_cache_miss(self, cache_name: str):
        """Track cache miss."""
        self.metrics.counter(f"cache.misses.total", {"cache": cache_name}).increment()

    def track_database_query(self, backend: str, operation: str, duration: float):
        """Track database query."""
        self.metrics.timer(
            f"database.query.duration", {"backend": backend, "operation": operation}
        ).record_duration(duration)

        self.metrics.counter(
            f"database.queries.total", {"backend": backend, "operation": operation}
        ).increment()

    def track_embedding_generation(self, provider: str, count: int, duration: float):
        """Track embedding generation."""
        self.metrics.timer(
            f"embeddings.generation.duration", {"provider": provider}
        ).record_duration(duration)

        self.metrics.histogram(f"embeddings.batch_size", tags={"provider": provider}).observe(count)

    def track_llm_request(self, provider: str, model: str, tokens: int, duration: float):
        """Track LLM request."""
        self.metrics.timer(
            f"llm.requests.duration", {"provider": provider, "model": model}
        ).record_duration(duration)

        self.metrics.histogram(
            f"llm.tokens.total", tags={"provider": provider, "model": model}
        ).observe(tokens)
    ...
    def get_component_health(self, component: str) -> Dict[str, Any]:
        ...
        component_metrics = {
            name: metric for name, metric in summary.items() if name.startswith(f"{component}.")
        }
    ...
    def get_system_overview(self) -> Dict[str, Any]:
        ...
        overview = {"total_metrics": len(summary), "components": {}, "system_health": "unknown"}
