            ...
            for conn in self._connections:
                if conn.state == ConnectionState.IDLE and conn.is_healthy():
                    ...
                    self.metrics.avg_connection_time = (
                        (self.metrics.avg_connection_time + (time.time() - start_time)) / 2
                    )
                    return conn
        ...
        async with self._lock:
            if connection.state == ConnectionState.BROKEN:
                ...
                self.metrics.avg_idle_time = (
                    (self.metrics.avg_idle_time + (idle_start - connection.last_used)) / 2
                )
...
class ConnectionPoolManager:
    ...
    async def create_pool(self, name: str, factory: ConnectionFactory, 
                         config: PoolConfig) -> ConnectionPool:
