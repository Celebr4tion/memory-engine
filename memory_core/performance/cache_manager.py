...
class CacheManager:
    ...
    def _evict_lru(self):
        ...
        with self._lock:
            while self.stats.memory_usage > target_memory and len(self._cache) > 0:
                # Remove oldest item
    ...
    def _should_evict(self, new_size: int) -> bool:
        ...
        return (
            self.stats.memory_usage + new_size > max_memory
            or self.stats.item_count >= self.config.max_items
        )
    ...
    def _make_space(self, needed_size: int) -> bool:
        ...
        while self.stats.memory_usage + needed_size > max_memory and len(self._cache) > 0:
            key, entry = self._cache.popitem(last=False)
            self.stats.memory_usage -= entry.size
            self.stats.item_count -= 1
            self.stats.evictions += 1
