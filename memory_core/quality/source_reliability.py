...
class SourceIdentifier:
    ...
    def __init__(self):
        ...
        self.authority_indicators = {
            ...
            "medium": ["reviewed", "editorial", "professional", "certified", "accredited"],
            "low": ["opinion", "personal", "unofficial", "unverified", "amateur"],
        }
...
class AuthorityAnalyzer:
    ...
    def __init__(self, query_engine: AdvancedQueryEngine):
        ...
        self.authority_database = {
            ...
            "questionable": ["tabloid", "conspiracy", "unverified", "clickbait"],
        }
    ...
    def _calculate_authority_confidence(
        self, source_identifier: str, source_type: SourceType, evidence_count: int
    ) -> float:
        ...
        if any(
            domain in source_identifier.lower()
            for domain in self.authority_database["high"] + self.authority_database["medium"]
        ):
            confidence += 0.3
...
class ConsistencyAnalyzer:
    ...
    def analyze_consistency(self, source_identifier: str, node: KnowledgeNode) -> ReliabilityMetric:
        """
...
class SourceReliabilityEngine:
    ...
    def assess_source_reliability(self, node: KnowledgeNode) -> SourceReliabilityScore:
        ...
        try:
            ...
            self.logger.info(
                f"Source reliability assessed: {reliability_level.value} ({overall_score:.2f})"
            )
    ...
    def _cache_score(self, source_identifier: str, score: SourceReliabilityScore):
        ...
        self.source_cache[source_identifier] = {"score": score, "timestamp": datetime.now()}

        # Limit cache size
        if len(self.source_cache) > 1000:
            # Remove oldest entries
            oldest_key = min(
                self.source_cache.keys(), key=lambda k: self.source_cache[k]["timestamp"]
            )
    ...
    def _generate_improvement_suggestions(
        self, metrics: Dict[str, ReliabilityMetric], source_type: SourceType
    ) -> List[str]:
    ...
    def _calculate_summary_statistics(
        self, scores: List[SourceReliabilityScore]
    ) -> Dict[str, float]:
        ...
        return {
            ...
            "high_reliability_rate": sum(
                1
                for s in scores
                if s.reliability_level
                in [ReliabilityLevel.HIGHLY_RELIABLE, ReliabilityLevel.RELIABLE]
            )
            / len(scores),
        }
    ...
    def _generate_global_recommendations(self, scores: List[SourceReliabilityScore]) -> List[str]:
        ...
        unreliable_count = sum(
            1
            for score in scores
            if score.reliability_level
            in [ReliabilityLevel.UNRELIABLE, ReliabilityLevel.QUESTIONABLE]
        )

        if unreliable_count > len(scores) * 0.3:
            recommendations.append(
                "Improve source quality - significant portion of sources are unreliable"
            )
        ...
        if user_generated_ratio > 0.5:
            recommendations.append(
                "Diversify source types - reduce reliance on user-generated content"
            )
        ...
        if low_confidence_count > len(scores) * 0.4:
            recommendations.append(
                "Improve source metadata and documentation for better assessment"
            )
    ...
    def _update_statistics(self, score: SourceReliabilityScore):
        ...
        self.stats["avg_reliability_score"] = (total_score + score.overall_score) / self.stats[
            "sources_assessed"
        ]
