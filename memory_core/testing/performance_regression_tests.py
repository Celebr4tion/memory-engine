...
class PerformanceRegressionTester:
    ...
    def __init__(self, 
        ...
        self.logger.info(f"Performance regression tester initialized with {len(self.baseline_results)} baselines")
    ...
    def create_query_benchmark(self, 
                             name: str, 
                             query_function: Callable,
                             expected_max_time_ms: float = 1000.0,
                             description: str = "") -> PerformanceBenchmark:
    ...
    def create_ingestion_benchmark(self,
                                 name: str,
    ...
    async def run_benchmark_suite(self, 
                                ...
                                benchmark_names: Optional[List[str]] = None) -> Dict[str, BenchmarkResult]:
        """
    ...
    async def _run_single_benchmark(self, benchmark: PerformanceBenchmark) -> BenchmarkResult:
        ...
        try:
            # Warmup runs
            self.logger.debug(f"Running {benchmark.warmup_runs} warmup iterations for {benchmark.name}")
    ...
    def detect_regressions(self, 
                          current_results: Optional[Dict[str, BenchmarkResult]] = None) -> List[RegressionReport]:
        """
        ...
        for benchmark_name, current_result in current_results.items():
            ...
            is_regression = (
                time_regression > self.regression_threshold or 
    ...
    def _generate_text_report(self, reports: List[RegressionReport]) -> str:
        ...
        if regressions:
            lines.extend([
                f"⚠️  REGRESSIONS DETECTED: {len(regressions)}",
                ""
            ])
