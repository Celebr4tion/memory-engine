...
class QueryCache:
    ...
    def __init__(
        ...
        self._cleanup_executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="cache-cleanup"
        )
        ...
        self.logger.info(
            f"Query cache initialized: max_size={max_size_mb}MB, max_entries={max_entries}, ttl={default_ttl}s"
        )
    ...
    def put(self, request: QueryRequest, response: QueryResponse) -> bool:
        ...
        with self._lock:
            ...
            self.logger.debug(
                f"Cached query result: {cache_key[:8]} (size: {estimated_size} bytes, TTL: {ttl}s)"
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_entry_size = (
                (self.stats.size_bytes / self.stats.entry_count)
