...
from .query_types import QueryRequest, QueryType, FilterCondition, GraphPattern, QueryExplanationStep
...
class QueryOptimizer:
    ...
    def optimize_query(self, request: QueryRequest) -> Tuple[QueryRequest, QueryPlan, List[QueryExplanationStep]]:
        """
        ...
        plan_step = QueryExplanationStep(
            ...
            operation="plan",
            details={"estimated_cost": execution_plan.estimated_cost, "parallel": execution_plan.parallel_execution}
        )
        ...
        self.logger.info(f"Query optimization completed in {total_time:.2f}ms, {len(applied_optimizations)} optimizations applied")
    ...
    def _analyze_query_pattern(self, request: QueryRequest) -> str:
        ...
        if request.filters:
            ...
            pattern_components.append(f"filters:{len(request.filters)}:{','.join(sorted(set(filter_types)))}")
    ...
    def _apply_optimizations(self, request: QueryRequest, pattern: str) -> QueryRequest:
        ...
        if request.query_type == QueryType.SEMANTIC_SEARCH:
            if request.similarity_threshold > 0.9:
                ...
                optimized.similarity_threshold = 0.6   # Slightly more restrictive for better precision
        ...
        if request.query_type == QueryType.AGGREGATION:
            ...
            optimized.cache_ttl = min(1800, request.cache_ttl)  # Shorter cache for semantic searches
    ...
    def _get_applied_optimizations(self, original: QueryRequest, optimized: QueryRequest) -> List[str]:
        """
    ...
    def get_optimization_suggestions(self, pattern: str) -> List[str]:
        ...
        if self.pattern_frequency[pattern] > 10:
            suggestions.append(f"Consider creating an optimized query template for pattern '{pattern}' (used {self.pattern_frequency[pattern]} times)")
        ...
        if pattern in self.performance_history:
            ...
            if avg_time > 1000:  # More than 1 second
                suggestions.append("Consider adding indexes or limiting result size for better performance")
        
        # Index suggestions
        if self.suggested_indexes:
            suggestions.append(f"Consider creating indexes: {', '.join(list(self.suggested_indexes)[:3])}")
