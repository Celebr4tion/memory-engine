...
class FilterProcessor:
    ...
    def _evaluate_filters(
        self, result: QueryResult, filters: List[FilterCondition], context: FilterContext
    ) -> bool:
        """
    ...
    def _evaluate_single_filter(
        self, result: QueryResult, filter_cond: FilterCondition, context: FilterContext
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(
                f"Error evaluating filter {filter_cond.field} {filter_cond.operator} {filter_cond.value}: {e}"
            )
...
class AggregationProcessor:
    ...
    def process_aggregations(
        self, results: List[QueryResult], aggregations: List[AggregationRequest]
    ...
    def _agg_count(
        self, results: List[QueryResult], request: AggregationRequest
    ) -> List[AggregationResult]:
        """Count aggregation."""
        return [
            AggregationResult(aggregation_type="count", field=request.field, value=len(results))
        ]
    ...
    def _agg_sum(
        ...
        return [
            AggregationResult(
                aggregation_type="sum", field=request.field, value=sum(values), count=len(values)
            )
        ]
    ...
    def _agg_average(
        ...
        return [
            AggregationResult(
                aggregation_type="avg", field=request.field, value=avg_value, count=len(values)
            )
        ]
    ...
    def _agg_min(
        ...
        return [
            AggregationResult(
                aggregation_type="min", field=request.field, value=min_value, count=len(values)
            )
        ]
    ...
    def _agg_max(
        ...
        return [
            AggregationResult(
                aggregation_type="max", field=request.field, value=max_value, count=len(values)
            )
        ]
