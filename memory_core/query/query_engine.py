...
class AdvancedQueryEngine:
    ...
    def query(self, request: QueryRequest) -> QueryResponse:
        ...
        try:
            ...
            if optimized_request.filters:
                ...
                raw_results = self.filter_processor.apply_filters(
                    raw_results, optimized_request.filters
                )
            ...
            paginated_results, total_count = self._apply_pagination(
                ranked_results, optimized_request
            )
            ...
            response = QueryResponse(
                ...
                next_offset=(
                    optimized_request.offset + len(paginated_results)
                    if (optimized_request.offset + len(paginated_results)) < total_count
                    else None
                ),
            )
            ...
            if optimized_request.explain:
                response.explanation = self.explainer.generate_explanation(
                    request=request, response=response, execution_steps=explanation_steps
                )
            ...
            self.logger.info(
                f"Query [{query_id}] completed in {total_time:.2f}ms, {len(paginated_results)} results"
            )
            return response
            ...
            return self._create_error_response(
                "Database connection failed", query_id, start_time, str(e)
            )
        except TimeoutError as e:
            self.logger.error(f"Query [{query_id}] failed - Query timeout: {e}")
            return self._create_error_response(
                "Query execution timeout", query_id, start_time, str(e)
            )
        except ValueError as e:
            self.logger.error(f"Query [{query_id}] failed - Invalid query parameters: {e}")
            return self._create_error_response(
                "Invalid query parameters", query_id, start_time, str(e)
            )
        except Exception as e:
            self.logger.error(f"Query [{query_id}] failed - Unexpected error: {e}")
            return self._create_error_response(
                "Internal server error", query_id, start_time, str(e)
            )
    ...
    def _execute_relationship_search(self, request: QueryRequest) -> List[QueryResult]:
        ...
        try:
            ...
            nodes = self.graph_adapter.find_nodes_by_content(
                request.query, limit=request.limit or 20
            )
    ...
    def _execute_text_search(self, request: QueryRequest) -> List[QueryResult]:
        ...
        try:
            nodes = self.graph_adapter.find_nodes_by_content(
                request.query, limit=request.limit or 50
            )
    ...
    def _create_error_response(
        self, error_message: str, query_id: str, start_time: float, error_details: str = ""
    ) -> QueryResponse:
        """
    ...
    def _update_performance_stats(self, execution_time_ms: float):
        ...
        if execution_time_ms < 100:
            self.stats.performance_metrics["fast_queries"] = (
                self.stats.performance_metrics.get("fast_queries", 0) + 1
            )
        elif execution_time_ms < 1000:
            self.stats.performance_metrics["medium_queries"] = (
                self.stats.performance_metrics.get("medium_queries", 0) + 1
            )
        else:
            self.stats.performance_metrics["slow_queries"] = (
                self.stats.performance_metrics.get("slow_queries", 0) + 1
            )
    ...
    def query_with_quality_enhancement(self, request: QueryRequest) -> QueryResponse:
        ...
        if self.quality_enhancement_engine:
            ...
            self.logger.warning(
                "Quality enhancement engine not available, falling back to standard query"
            )
