...
class QueryExplainer:
    ...
    def generate_natural_language_explanation(
        self, explanation: QueryExplanation, context: Optional[ExplanationContext] = None
    ) -> str:
        """
    ...
    def generate_visual_execution_plan(self, explanation: QueryExplanation) -> Dict[str, Any]:
        ...
        for i, step in enumerate(explanation.execution_plan):
            step_data = {
                ...
                "time_percentage": (
                    (step.execution_time_ms / explanation.total_execution_time_ms * 100)
                    if explanation.total_execution_time_ms > 0
                    else 0
                ),
    ...
    def analyze_performance_bottlenecks(
        self, explanation: QueryExplanation
    ) -> List[Dict[str, Any]]:
        """
    ...
    def generate_query_optimization_suggestions(
        self, request: QueryRequest, explanation: QueryExplanation
    ) -> List[str]:
        """
        ...
        if request.query_type == QueryType.NATURAL_LANGUAGE:
            suggestions.append(
                "Consider using more specific query types (semantic_search, graph_pattern) for better performance"
            )
        ...
        if request.include_relationships and request.max_depth > 2:
            suggestions.append(
                "Consider reducing max_depth for relationship traversal to improve performance"
            )

        if len(request.filters) == 0 and explanation.total_execution_time_ms > 500:
            suggestions.append(
                "Add filters to reduce the search space and improve query performance"
            )

        # Performance-based suggestions
        if explanation.total_execution_time_ms > 2000:
            suggestions.append(
                "Query is slow (>2s). Consider breaking it into smaller, more specific queries"
            )
        ...
        if execution_plan:
            ...
            if result_step and result_step.output_size and result_step.output_size > 1000:
                suggestions.append(
                    "Large result set detected. Consider adding more selective filters"
                )
    ...
    def _generate_translation_steps(
        self, request: QueryRequest, execution_steps: List[QueryExplanationStep]
    ) -> List[str]:
        ...
        if request.query_type == QueryType.NATURAL_LANGUAGE:
            ...
            nlp_step = next(
                (s for s in execution_steps if s.step_name == "natural_language_processing"), None
            )
        ...
        # Filtering
        filter_step = next(
            (s for s in execution_steps if s.step_name == "filter_application"), None
        )
        if filter_step:
    ...
    def _explain_execution_overview(
        self, explanation: QueryExplanation, context: ExplanationContext
    ) -> str:
    ...
    def _explain_performance_analysis(self, explanation: QueryExplanation) -> str:
        ...
        slowest_step = max(
            explanation.execution_plan, key=lambda s: s.execution_time_ms or 0, default=None
        )
    ...
    def _explain_result_summary(self, explanation: QueryExplanation) -> str:
        ...
        exec_step = next(
            (s for s in explanation.execution_plan if s.step_name == "query_execution"), None
        )
        ...
        rank_step = next(
            (s for s in explanation.execution_plan if s.step_name == "result_ranking"), None
        )
