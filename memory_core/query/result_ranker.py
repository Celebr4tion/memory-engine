...
class RankingCriteria:
    ...
    def __post_init__(self):
        ...
        total_weight = (self.relevance_weight + self.quality_weight + 
                       self.freshness_weight + self.popularity_weight + 
                       self.diversity_weight + sum(self.custom_weights.values()))
...
class ResultRanker:
    ...
    def rank_results(self, 
                    ...
                    criteria: Optional[RankingCriteria] = None) -> List[QueryResult]:
        """
        ...
        self.logger.info(f"Ranking completed. Top result score: {ranked_results[0].combined_score:.3f}")
        return ranked_results
    
    def _extract_ranking_features(self, 
                                 result: QueryResult, 
                                 request: QueryRequest,
                                 all_results: List[QueryResult]) -> RankingFeatures:
        """
    ...
    def _calculate_popularity_score(self, result: QueryResult) -> float:
        ...
        popularity_base = math.log(access_count + 1) / math.log(100)  # Normalize to common access ranges
    ...
    def _calculate_diversity_score(self, result: QueryResult, all_results: List[QueryResult]) -> float:
        """
    ...
    def _calculate_combined_score(self, features: RankingFeatures, criteria: RankingCriteria) -> float:
        """
    ...
    def _apply_diversity_filtering(self, results: List[QueryResult], request: QueryRequest) -> List[QueryResult]:
        """
    ...
    def _apply_user_preferences(self, criteria: RankingCriteria, preferences: Dict[str, float]) -> RankingCriteria:
        """
        ...
        total = (criteria.relevance_weight + criteria.quality_weight + 
                criteria.freshness_weight + criteria.popularity_weight + criteria.diversity_weight)
        if total > 0:
