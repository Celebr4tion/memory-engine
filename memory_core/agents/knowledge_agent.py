...
class KnowledgeAgent(BaseAgent):
    ...
    async def extract_and_store_knowledge(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for unit in knowledge_units:
                ...
                node_ids.append(
                    {"id": node_id, "content": unit["content"], "tags": unit.get("tags", [])}
                )
        ...
        try:
            ...
            for node_id in similar_node_ids:
                try:
                    ...
                    outgoing = []
                    try:
                        rel_outgoing = self.knowledge_engine.graph.get_outgoing_relationships(
                            node_id
                        )
                    ...
                    incoming = []
                    try:
                        rel_incoming = self.knowledge_engine.graph.get_incoming_relationships(
                            node_id
                        )
    ...
    async def create_relationship(
        self, from_id: str, to_id: str, relation_type: str, confidence_score: float = 0.8
    ) -> Dict[str, Any]:
        """
...
def create_knowledge_agent(
    ...
    knowledge_engine = KnowledgeEngine(host=host, port=port, enable_versioning=enable_versioning)

    # Create the knowledge agent
    agent = KnowledgeAgent(knowledge_engine=knowledge_engine, model=model)
