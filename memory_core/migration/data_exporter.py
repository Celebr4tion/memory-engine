...
class DataExporter:
    ...
    async def export_knowledge_graph(self, engine: Any, output_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            relationships = await self._get_all_relationships(engine) if self.config.include_relationships else []
    ...
    async def _export_json_split(self, data: Dict, output_path: str):
        ...
        rel_files = await self._split_and_export_json(relationships, f"{base_path}_relationships", "relationships")
        
        self._file_count = 1 + len(node_files) + len(rel_files)
    
    async def _split_and_export_json(self, items: List[Dict], base_path: str, item_type: str) -> List[str]:
        ...
        for i, node in enumerate(nodes):
            ...
            for key, value in node.items():
                ...
                data_elem = ET.SubElement(node_elem, "data", {
                    "key": key_mapping.get(f"node_{key}", f"d{key_id}")
                })
        ...
        for i, rel in enumerate(relationships):
            ...
            for key, value in rel.items():
                ...
                data_elem = ET.SubElement(edge_elem, "data", {
                    "key": key_mapping.get(f"edge_{key}", f"d{key_id}")
                })
    ...
    async def _export_networkx(self, nodes: List[Dict], relationships: List[Dict], output_path: str):
