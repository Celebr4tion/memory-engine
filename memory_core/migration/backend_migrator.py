...
class BackendMigrator:
    ...
    async def migrate(self, source_engine: Any, target_engine: Any) -> MigrationResult:
        ...
        try:
            ...
            if self.config.strategy == MigrationStrategy.INCREMENTAL:
                target_count = await self._migrate_incremental(source_engine, target_engine, source_count)
            ...
            if self.config.verify_migration:
                await self._verify_migration(source_engine, target_engine, source_count, target_count)
    ...
    async def _migrate_incremental(self, source_engine: Any, target_engine: Any, 
                                  total_count: int) -> int:
    ...
    async def _verify_migration(self, source_engine: Any, target_engine: Any, 
...
async def migrate_janusgraph_to_sqlite(janusgraph_engine: Any, sqlite_engine: Any,
...
async def migrate_sqlite_to_json(sqlite_engine: Any, json_engine: Any,
                                **kwargs) -> MigrationResult:
...
async def migrate_json_to_janusgraph(json_engine: Any, janusgraph_engine: Any,
                                    **kwargs) -> MigrationResult:
