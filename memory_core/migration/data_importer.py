...
class DataImporter:
    ...
    async def import_from_file(self, file_path: str, engine: Any, 
                              file_format: str = None) -> Dict[str, Any]:
    ...
    async def import_from_directory(self, directory_path: str, engine: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            patterns = ['*.json', '*.csv', '*.xml', '*.graphml', '*.cypher', '*.gremlin', '*.rdf', '*.pkl']
            files = []
    ...
    async def _import_csv(self, file_path: str, engine: Any):
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if any(field in first_line.lower() for field in ['source_id', 'target_id', 'from_id', 'to_id']):
