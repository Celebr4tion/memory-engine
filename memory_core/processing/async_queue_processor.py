    ...
    async def process(self, task: QueueTask) -> Any:
        ...
        try:
            ...
            embeddings = await loop.run_in_executor(
                None, self.embedding_manager.generate_embeddings, texts, task_type
            )
    ...
    async def process(self, task: QueueTask) -> Any:
        ...
        try:
            ...
            knowledge_units = await loop.run_in_executor(
                None, self.extractor.extract_knowledge_units, content
            )
...
class RelationshipExtractionProcessor(TaskProcessor):
    ...
    async def process(self, task: QueueTask) -> Any:
        ...
        try:
            ...
            return {"metrics": metrics, "relationships_created": metrics.relationships_created}
...
class AsyncProcessingQueue:
    ...
    def __init__(
        self, max_workers: int = 4, max_queue_size: int = 1000, task_timeout: float = 300.0
    ):
        """
    ...
    async def _cleanup_completed_tasks(self, max_completed: int = 1000):
        ...
        if len(self.completed_tasks) > max_completed:
            ...
            sorted_tasks = sorted(
                self.completed_tasks.items(), key=lambda x: x[1].completed_at or 0
            )
    ...
    def get_queue_metrics(self) -> QueueMetrics:
        ...
        with self._metrics_lock:
            return QueueMetrics(
                ...
                pending_tasks=(
                    len(self.pending_queue._queue)
                    if hasattr(self.pending_queue, "_queue")
                    else self.metrics.pending_tasks
                ),
...
async def create_knowledge_processing_queue(
    storage=None, embedding_manager=None, extractor=None, relationship_extractor=None, **kwargs
