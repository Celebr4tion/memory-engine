...
class PerspectiveExtractor:
    ...
    def __init__(self, query_engine: AdvancedQueryEngine):
        ...
        self.perspective_indicators = {
            PerspectiveType.OPPOSING: [
                'however', 'but', 'on the contrary', 'conversely', 'disagreed',
                'opposed', 'contradicts', 'refutes', 'challenges', 'disputes'
            ],
            PerspectiveType.COMPLEMENTARY: [
                'additionally', 'furthermore', 'moreover', 'also', 'supports',
                'confirms', 'validates', 'complements', 'builds upon'
            ],
            PerspectiveType.ALTERNATIVE: [
                'alternatively', 'another approach', 'different method',
                'another way', 'alternatively', 'instead', 'rather than'
            ],
            PerspectiveType.STAKEHOLDER: [
                'experts believe', 'users think', 'researchers suggest',
                'practitioners argue', 'stakeholders view', 'community feels'
            ],
            PerspectiveType.METHODOLOGICAL: [
                'methodology', 'approach', 'technique', 'method',
                'framework', 'model', 'strategy', 'paradigm'
            ]
        }
    
    def extract_perspectives(self, topic: str, 
                           perspective_types: List[PerspectiveType] = None,
                           min_evidence_count: int = 2) -> List[Perspective]:
        """
    ...
    def _extract_type_specific_perspectives(self, topic: str, 
                                          topic_data: List[Dict[str, Any]],
                                          perspective_type: PerspectiveType,
                                          min_evidence_count: int) -> List[Perspective]:
        """Extract perspectives for a specific type."""
        perspectives = []
        
        try:
            if perspective_type == PerspectiveType.OPPOSING:
                perspectives.extend(self._extract_opposing_perspectives(topic, topic_data, min_evidence_count))
            elif perspective_type == PerspectiveType.COMPLEMENTARY:
                perspectives.extend(self._extract_complementary_perspectives(topic, topic_data, min_evidence_count))
            elif perspective_type == PerspectiveType.ALTERNATIVE:
                perspectives.extend(self._extract_alternative_perspectives(topic, topic_data, min_evidence_count))
            elif perspective_type == PerspectiveType.STAKEHOLDER:
                perspectives.extend(self._extract_stakeholder_perspectives(topic, topic_data, min_evidence_count))
            elif perspective_type == PerspectiveType.METHODOLOGICAL:
                perspectives.extend(self._extract_methodological_perspectives(topic, topic_data, min_evidence_count))
            elif perspective_type == PerspectiveType.TEMPORAL:
                perspectives.extend(self._extract_temporal_perspectives(topic, topic_data, min_evidence_count))
    ...
    def _extract_opposing_perspectives(self, topic: str, 
                                     topic_data: List[Dict[str, Any]],
                                     min_evidence_count: int) -> List[Perspective]:
    ...
    def _extract_complementary_perspectives(self, topic: str, 
                                          topic_data: List[Dict[str, Any]],
                                          min_evidence_count: int) -> List[Perspective]:
            ...
            for indicator in self.perspective_indicators[PerspectiveType.COMPLEMENTARY]:
                if indicator in content:
                    ...
                    if viewpoint:
                        ...
                        group_key = self._find_semantic_group(viewpoint, complementary_groups.keys())
                        if not group_key:
    ...
    def _extract_alternative_perspectives(self, topic: str, 
                                        topic_data: List[Dict[str, Any]],
                                        min_evidence_count: int) -> List[Perspective]:
        ...
        for group_key, group_items in alternative_groups.items():
            if len(group_items) >= min_evidence_count:
                ...
                perspective = Perspective(
                    ...
                    metadata={
                        'approaches': list(set(item['approach'] for item in group_items))
                    },
                    key_claims=key_claims,
                    evidence_quality=self._assess_evidence_quality(group_items)
                )
    ...
    def _extract_stakeholder_perspectives(self, topic: str, 
                                        topic_data: List[Dict[str, Any]],
                                        min_evidence_count: int) -> List[Perspective]:
    ...
    def _extract_methodological_perspectives(self, topic: str, 
                                           topic_data: List[Dict[str, Any]],
                                           min_evidence_count: int) -> List[Perspective]:
    ...
    def _extract_temporal_perspectives(self, topic: str, 
                                     topic_data: List[Dict[str, Any]],
                                     min_evidence_count: int) -> List[Perspective]:
            ...
            if len(year_data) >= min_evidence_count:
                ...
                if viewpoint:
                    ...
                    perspective = Perspective(
                        ...
                        viewpoint=viewpoint,
                        supporting_evidence=evidence_ids,
                        confidence_score=len(year_data) / max(len(temporal_groups.values()), key=len),
                        ...
                        key_claims=key_claims,
                        evidence_quality=sum(item.get('relevance_score', 0.5) for item in year_data) / len(year_data)
                    )
                    perspectives.append(perspective)
    ...
    def _extract_viewpoint_around_indicator(self, content: str, indicator: str) -> Optional[str]:
        ...
        for sentence in sentences:
            if indicator.lower() in sentence.lower():
                ...
                if (sentence_idx + 1 < len(sentences) and 
                    len(sentences[sentence_idx + 1]) < 200):
    ...
    def _generate_perspective_key(self, viewpoint: str) -> str:
        ...
        stop_words = {
            ...
            'will', 'would', 'could', 'should', 'must', 'might', 'also',
            'some', 'many', 'most', 'very', 'more', 'than', 'such', 'when',
            'where', 'what', 'which', 'their', 'there', 'these', 'those'
        }
    ...
    def _assess_evidence_quality(self, group_items: List[Dict[str, Any]]) -> float:
        ...
        for item in group_items:
            ...
            quality = (length_score * 0.3 + relevance_score * 0.4 + 
                      metadata_score * 0.2 + relationship_score * 0.1)
...
class PerspectiveComparator:
    ...
    def compare_perspectives(self, topic: str, 
                           perspectives: List[Perspective]) -> PerspectiveComparison:
        """
    ...
    def _create_comparison_matrix(self, perspectives: List[Perspective]) -> Dict[str, Dict[str, float]]:
    ...
    def _calculate_perspective_similarity(self, p1: Perspective, p2: Perspective) -> float:
        ...
        # Combined similarity
        similarity = (
            viewpoint_similarity * 0.5 +
            claims_similarity * 0.3 +
    ...
    def _are_perspectives_opposing(self, p1: Perspective, p2: Perspective) -> bool:
        ...
        opposing_keywords = [
            ('positive', 'negative'), ('good', 'bad'), ('effective', 'ineffective'),
            ('beneficial', 'harmful'), ('increase', 'decrease'), ('support', 'oppose'),
            ('agree', 'disagree'), ('should', 'should not'), ('yes', 'no')
        ]
        ...
        if (p1.perspective_type == PerspectiveType.OPPOSING or 
            p2.perspective_type == PerspectiveType.OPPOSING):
            return True
    ...
    def _create_disagreement_statement(self, p1: Perspective, p2: Perspective) -> str:
        ...
        return (f"There is disagreement between {p1.perspective_type.value} and "
               f"{p2.perspective_type.value} perspectives on key aspects")
    
    def _determine_consensus_level(self, perspectives: List[Perspective], 
                                 comparison_matrix: Dict[str, Dict[str, float]]) -> ConsensusLevel:
    ...
    def _generate_synthesis_summary(self, topic: str, perspectives: List[Perspective],
                                  consensus_areas: List[str], 
                                  disagreement_areas: List[str]) -> str:
    ...
    def _calculate_analysis_confidence(self, perspectives: List[Perspective]) -> float:
        ...
        avg_perspective_confidence = sum(p.confidence_score for p in perspectives) / len(perspectives)
    ...
    def _create_single_perspective_comparison(self, topic: str, 
                                            perspectives: List[Perspective]) -> PerspectiveComparison:
    ...
    def _create_error_comparison(self, topic: str, perspectives: List[Perspective], 
                               error_msg: str) -> PerspectiveComparison:
...
class PerspectiveAnalysisEngine:
    ...
    def analyze_perspectives(self, topic: str,
                           perspective_types: List[PerspectiveType] = None,
                           include_temporal_analysis: bool = True,
                           include_stakeholder_analysis: bool = True) -> PerspectiveAnalysisReport:
        """
        ...
        try:
            ...
            if include_stakeholder_analysis:
                stakeholder_analysis = self._perform_stakeholder_analysis(
                    topic, perspectives
                )
            ...
            self.logger.info(f"Perspective analysis completed: {len(perspectives)} perspectives, "
    ...
    def _perform_stakeholder_analysis(self, topic: str, 
                                    perspectives: List[Perspective]) -> List[StakeholderPerspective]:
    ...
    def _extract_stakeholder_interests(self, stakeholder_group: str, 
                                     perspective: Perspective) -> List[str]:
    ...
    def _extract_stakeholder_concerns(self, stakeholder_group: str, 
                                    perspective: Perspective) -> List[str]:
        ...
        for indicator in concern_indicators:
            if indicator in viewpoint:
                ...
                concern_context = self._extract_context_around_word(perspective.viewpoint, indicator)
                if concern_context:
                    concerns.append(concern_context)
        
        return concerns[:3]  # Top 3 concerns
    
    def _extract_stakeholder_priorities(self, stakeholder_group: str, 
                                      perspective: Perspective) -> List[str]:
    ...
    def _extract_context_around_word(self, text: str, word: str, context_size: int = 50) -> Optional[str]:
    ...
    def _analyze_temporal_evolution(self, topic: str, 
                                  perspectives: List[Perspective]) -> Optional[TemporalPerspectiveEvolution]:
        """Analyze how perspectives have evolved over time."""
        # Filter temporal perspectives
        temporal_perspectives = [p for p in perspectives if p.perspective_type == PerspectiveType.TEMPORAL]
        ...
        try:
            ...
            for i, perspective in enumerate(temporal_perspectives):
                change = {
                    ...
                    'key_changes': self._identify_key_changes(perspective, temporal_perspectives[:i])
                }
    ...
    def _identify_turning_points(self, temporal_perspectives: List[Perspective]) -> List[Dict[str, Any]]:
    ...
    def _determine_current_trajectory(self, temporal_perspectives: List[Perspective]) -> str:
        ...
        recent_perspectives = temporal_perspectives[-3:] if len(temporal_perspectives) >= 3 else temporal_perspectives[-2:]
    ...
    def _identify_key_changes(self, current_perspective: Perspective, 
                            previous_perspectives: List[Perspective]) -> List[str]:
    ...
    def _determine_overall_consensus(self, comparisons: List[PerspectiveComparison]) -> ConsensusLevel:
    ...
    def _generate_insights(self, perspectives: List[Perspective], 
                          comparisons: List[PerspectiveComparison]) -> List[str]:
        ...
        if perspectives:
            ...
            insights.append(f"Perspectives are dominated by {most_common_type[0]} viewpoints ({most_common_type[1]} out of {len(perspectives)})")
            
            # Quality insights
            high_confidence_count = len([p for p in perspectives if p.confidence_score > 0.7])
            if high_confidence_count > len(perspectives) * 0.5:
                insights.append(f"High confidence in {high_confidence_count} perspectives indicates strong evidence base")
        ...
        stakeholder_perspectives = [p for p in perspectives if p.perspective_type == PerspectiveType.STAKEHOLDER]
        if len(stakeholder_perspectives) >= 2:
            insights.append(f"Multiple stakeholder viewpoints ({len(stakeholder_perspectives)}) provide comprehensive coverage")
        
        return insights[:5]  # Top 5 insights
    
    def _generate_recommendations(self, perspectives: List[Perspective], 
                                comparisons: List[PerspectiveComparison]) -> List[str]:
        ...
        if perspectives:
            ...
            if len(type_counts) < 3:
                recommendations.append("Seek additional perspective types for more comprehensive analysis")
        
        # Consensus-based recommendations
        if comparisons:
            overall_comparison = next((comp for comp in comparisons if len(comp.perspectives) == len(perspectives)), None)
            
            if overall_comparison:
                if overall_comparison.consensus_level in [ConsensusLevel.STRONG_CONSENSUS, ConsensusLevel.MODERATE_CONSENSUS]:
                    ...
                    recommendations.append("Seek additional evidence to resolve areas of uncertainty")
        
        # Stakeholder recommendations
        stakeholder_perspectives = [p for p in perspectives if p.perspective_type == PerspectiveType.STAKEHOLDER]
        if len(stakeholder_perspectives) < 2:
            recommendations.append("Include additional stakeholder perspectives for balanced analysis")
    ...
    def _calculate_analysis_confidence(self, perspectives: List[Perspective], 
                                     comparisons: List[PerspectiveComparison]) -> float:
        ...
        if comparisons:
            comparison_confidence = sum(comp.confidence_in_analysis for comp in comparisons) / len(comparisons)
    ...
    def _update_statistics(self, report: PerspectiveAnalysisReport, start_time: float):
        ...
        self.stats['avg_processing_time_ms'] = (total_time + report.processing_time_ms) / self.stats['analyses_performed']
