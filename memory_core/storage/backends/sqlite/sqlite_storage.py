...
class SqliteStorage(GraphStorageInterface):
    ...
    async def update_node(self, node_id: str, properties: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            known_fields = ['content', 'source', 'creation_timestamp', 'rating_richness', 
                           'rating_truthfulness', 'rating_stability']
            update_fields = []
            values = []
    ...
    async def delete_node(self, node_id: str) -> bool:
        ...
        try:
            ...
            await self._db_connection.execute("DELETE FROM edges WHERE from_node_id = ? OR to_node_id = ?", 
                                            (node_id, node_id))
            
            # Delete the node
            cursor = await self._db_connection.execute("DELETE FROM nodes WHERE node_id = ?", (node_id,))
            await self._db_connection.commit()
    ...
    async def delete_edge(self, edge_id: str) -> bool:
        ...
        try:
            cursor = await self._db_connection.execute("DELETE FROM edges WHERE edge_id = ?", (edge_id,))
            await self._db_connection.commit()
    ...
    async def merge_nodes(self, node_id1: str, node_id2: str) -> bool:
        ...
        try:
            ...
            """, (node_id1, node_id2, node_id1))  # Avoid self-loops
