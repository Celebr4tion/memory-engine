...
class SentenceTransformersProvider(EmbeddingProviderInterface):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="sentence_transformers"
        )

        self.logger.info(
            f"Initialized SentenceTransformers provider with model: {self._model_name}, device: {self.device}"
        )
    ...
    def _load_model(self) -> SentenceTransformer:
        ...
        if self._model is None:
            try:
                ...
                model_kwargs = {"device": self.device, "trust_remote_code": self.trust_remote_code}
    ...
    def _detect_dimension(self) -> int:
        ...
        if self._model is None:
            raise EmbeddingProviderError(
                "Model must be loaded before detecting dimension", provider="sentence_transformers"
            )
    ...
    async def generate_embedding(
        ...
        try:
            ...
            embedding = await asyncio.get_event_loop().run_in_executor(
                self._executor, self._encode_single, text
            )

            self.logger.debug(
                f"Generated SentenceTransformers embedding of length {len(embedding)}"
            )
            return embedding
    ...
    async def generate_embeddings(
        ...
        try:
            ...
            self.logger.debug(
                f"Generating SentenceTransformers embeddings for {len(valid_texts)} texts"
            )
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            if len(test_embedding) != expected_dim:
                self.logger.error(
                    f"Dimension mismatch: expected {expected_dim}, got {len(test_embedding)}"
                )
                return False
