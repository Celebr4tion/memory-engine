...
class OllamaEmbeddingProvider(EmbeddingProviderInterface):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger.info(f"Initialized Ollama provider with model '{self._model_name}' at {self.base_url}")
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                try:
                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=self.timeout)) as session:
                        async with session.post(self.embeddings_endpoint, json=payload) as response:
                            if response.status == 200:
                                ...
                                self.logger.debug(f"Generated Ollama embedding of length {len(embedding)}")
                                ...
                                if attempt < self.max_retries - 1:
                                    self.logger.warning(f"Ollama request failed (attempt {attempt + 1}): {response.status} {error_text}")
                    ...
                    if attempt < self.max_retries - 1:
                        self.logger.warning(f"Ollama connection failed (attempt {attempt + 1}): {str(e)}")
                        await asyncio.sleep(self.retry_delay)
                        continue
                    else:
                        raise EmbeddingProviderError(
        ...
        for i in range(0, len(texts), self.max_batch_size):
            ...
            for j, text in enumerate(batch):
                try:
                    ...
                    self.logger.error(f"Failed to embed text at index {batch_start_idx + j}: {str(text_error)}")
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                async with session.get(self.tags_endpoint) as response:
                    ...
                    if self._model_name not in available_models:
                        self.logger.error(f"Model '{self._model_name}' not found. Available models: {available_models}")
                        return False
