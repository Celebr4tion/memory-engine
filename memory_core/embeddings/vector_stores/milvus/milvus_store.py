...
class MilvusVectorStore(VectorStoreInterface):
    ...
    async def add_vectors(
        ...
        try:
            ...
            data = [
                ids,  # id field
                [vector.tolist() if isinstance(vector, np.ndarray) else vector for vector in vectors],  # vector field
            ...
            if metadata and await self._collection_has_metadata(collection):
                if len(metadata) != len(vectors):
                    raise VectorStoreOperationError("Number of metadata entries must match number of vectors")
    ...
    async def search_vectors(
        ...
        try:
            ...
            search_params = {"metric_type": self._index_params["metric_type"], "params": {"nprobe": self.nprobe}}
            ...
            results = collection.search(
                data=[query_vector.tolist() if isinstance(query_vector, np.ndarray) else query_vector],
                anns_field="vector",
                param=search_params,
                limit=top_k,
    ...
    async def get_vector(
        ...
        try:
            ...
            results = collection.query(
                expr=f'id == "{vector_id}"',
                output_fields=["id", "vector", "metadata"] if await self._collection_has_metadata(collection) else ["id", "vector"]
            )
