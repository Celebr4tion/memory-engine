...
class EmbeddingManager:
    ...
    def __init__(
        self, vector_store: VectorStoreMilvus, cache_size: int = 1000, cache_ttl: int = 3600
    ):
        """
        ...
        if not api_key:
            raise ValueError(
                "GOOGLE_API_KEY not configured. Set it via environment variable or configuration file."
            )
    ...
    def generate_embedding(self, text: str, task_type: str = "SEMANTIC_SIMILARITY") -> List[float]:
        ...
        try:
            ...
            result = self.client.models.embed_content(
                model=self.embedding_model, contents=text, config=embedding_config
            )
            ...
            if hasattr(result, "embeddings"):
                ...
                if isinstance(result.embeddings, list) and result.embeddings:
                    ...
                    # object with .values attribute
                        ...
                        raise RuntimeError(
                            "Unexpected embedding result structure received from API."
                        )
                else:
    ...
    def search_similar_nodes(self, query_text: str, top_k: int = 5) -> List[str]:
        """
    ...
    def find_similar_nodes(
        self, query_embedding: List[float], top_k: int = 5, similarity_threshold: float = 0.7
    ...
    def clear_cache(self):
        ...
        self.cache = EmbeddingCache(
            max_entries=self.cache.max_entries, ttl_seconds=self.cache.ttl_seconds
        )
