...
from memory_core.embeddings.vector_stores.interfaces import VectorStoreInterface, VectorStoreError
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.embedding_provider = EmbeddingProviderFactory.create_provider(
                self.embedding_provider_type, self.embedding_provider_config
            )

            # Test embedding provider connection
            if not await self.embedding_provider.test_connection():
                self.logger.warning(
                    f"Embedding provider {self.embedding_provider_type} connection test failed"
                )
                return False
            ...
            self.vector_store = VectorStoreFactory.create_vector_store(
                self.vector_store_type, self.vector_store_config
            )
    ...
    async def store_embedding(
        ...
        try:
            ...
            vector_ids = await self.vector_store.add_vectors(
                vectors=[embedding], ids=[node_id], metadata=[metadata] if metadata else None
            )
    ...
    async def search_similar(
        ...
        try:
            ...
            results = await self.vector_store.search_vectors(
                query_vector=query_embedding, top_k=top_k, filter_expr=filter_expr
            )
    ...
    async def update_embedding(
        ...
        try:
            ...
            return await self.vector_store.update_vector(
                vector_id=node_id, vector=embedding, metadata=metadata
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self._initialized:
            ...
            try:
                ...
                health_status["vector_store"] = {"type": self.vector_store_type, **store_health}
            except Exception as e:
            ...
            if self.embedding_provider:
                info["embedding_provider_info"] = {
                    ...
                    "supported_task_types": [
                        t.value for t in self.embedding_provider.get_supported_task_types()
                    ],
                }
