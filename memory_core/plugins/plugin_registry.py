...
class PluginRegistry:
    ...
    def list_plugins(self, plugin_type: str = None, tags: List[str] = None) -> List[PluginMetadata]:
    ...
    def get_plugins_by_category(self, category: str) -> List[PluginMetadata]:
        ...
        return [metadata for metadata in self._plugins.values() if metadata.plugin_type == category]

    def get_plugins_by_tag(self, tag: str) -> List[PluginMetadata]:
        """Get plugins by tag."""
        return [metadata for metadata in self._plugins.values() if tag in metadata.tags]
    ...
    def validate_plugin_metadata(self, metadata: PluginMetadata) -> List[str]:
        ...
        if (
            ...
            and self._plugins[metadata.name].version != metadata.version
        ):
    ...
    async def publish_plugin(self, metadata: PluginMetadata, plugin_package_path: str) -> bool:
