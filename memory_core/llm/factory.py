...
class LLMProviderFactory:
    ...
    def _register_providers(self):
        ...
        try:
            from memory_core.llm.providers.huggingface.huggingface_provider import HuggingFaceLLMProvider
    ...
    def create_provider(self, provider_type: Optional[str] = None, 
                       config_override: Optional[Dict[str, Any]] = None) -> LLMProviderInterface:
        """
    ...
    def _get_provider_config(self, provider_type: str, config: Any, 
        ...
        if provider_type in api_key_mapping:
            ...
            if api_key:
                ...
                if provider_type != 'ollama':  # Ollama doesn't need API keys
                    self.logger.warning(f"No API key found for {provider_type} provider. "
                                      f"Set {env_var} environment variable.")
        
        return config
    
    def create_fallback_chain(self, primary_provider: str, 
                             fallback_providers: Optional[List[str]] = None,
                             config_override: Optional[Dict[str, Any]] = None) -> List[LLMProviderInterface]:
        """
        ...
        if not providers:
            raise RuntimeError("No LLM providers could be created. Check your configuration and dependencies.")
...
def create_fallback_chain(primary_provider: str, 
                         fallback_providers: Optional[List[str]] = None,
                         config_override: Optional[Dict[str, Any]] = None) -> List[LLMProviderInterface]:
    """
    Create a chain of LLM providers for fallback support.
