...
class LLMManager:
    ...
    async def connect(self) -> bool:
        ...
        if connected_count > 0:
            self.logger.info(
                f"LLM Manager connected with {connected_count}/{len(self.providers)} providers"
            )
            # Start health check task
    ...
    def _update_circuit_breaker(self, provider_name: str, success: bool):
        ...
        if success:
            ...
            if breaker["failure_count"] >= self.config.circuit_breaker_threshold:
                breaker["state"] = "open"
                self.logger.warning(
                    f"Circuit breaker for {provider_name} opened after {breaker['failure_count']} failures"
                )

    async def _try_provider(
        self, provider: LLMProviderInterface, operation: str, *args, **kwargs
    ) -> ProviderAttempt:
        """
        ...
        if self._is_circuit_breaker_open(provider_name):
            return ProviderAttempt(
                provider_name=provider_name, success=False, error="Circuit breaker open"
            )

        # Check if provider is available
        if not self.provider_status[provider_name]["available"]:
            return ProviderAttempt(
                provider_name=provider_name, success=False, error="Provider marked as unavailable"
            )
        ...
        try:
            ...
            response = await asyncio.wait_for(method(*args, **kwargs), timeout=self.config.timeout)
            ...
            self.logger.debug(
                f"Operation {operation} succeeded with {provider_name} in {response_time:.2f}s"
            )
    ...
    def _handle_provider_failure(self, provider_name: str, error_msg: str):
        ...
        if (
            self.provider_status[provider_name]["consecutive_failures"]
            >= self.config.circuit_breaker_threshold
        ):
            self.provider_status[provider_name]["available"] = False
            self.logger.warning(
                f"Provider {provider_name} marked as unavailable after {self.provider_status[provider_name]['consecutive_failures']} consecutive failures"
            )
    ...
    async def generate_completion(
        ...
        raise LLMError(
            f"All providers failed for completion generation: {'; '.join(error_details)}"
        )
    ...
    async def generate_chat_completion(
        ...
        error_details = [f"{a.provider_name}: {a.error}" for a in attempts]
        raise LLMError(
            f"All providers failed for chat completion generation: {'; '.join(error_details)}"
        )
    ...
    async def extract_knowledge_units(
        ...
        if self.config.fallback_strategy == FallbackStrategy.BEST_EFFORT:
            self.logger.warning(
                "All providers failed for knowledge extraction, returning empty list"
            )
            return []
    ...
    async def detect_relationships(
        ...
        for provider in self.providers:
            attempt = await self._try_provider(provider, "detect_relationships", entities, context)
            attempts.append(attempt)
        ...
        if self.config.fallback_strategy == FallbackStrategy.BEST_EFFORT:
            self.logger.warning(
                "All providers failed for relationship detection, returning empty list"
            )
            return []

        error_details = [f"{a.provider_name}: {a.error}" for a in attempts]
        raise LLMError(
            f"All providers failed for relationship detection: {'; '.join(error_details)}"
        )
            ...
            if metrics:
                summary[provider_name] = {
                    ...
                    - (
                        ...
                        / max(len(metrics) + self.failure_counts[provider_name], 1)
                    ),
                }
            else:
                summary[provider_name] = {
                ...
                for provider in self.providers:
                    ...
                    try:
                        ...
                        if self.provider_status[provider_name]["available"]:
                            self.provider_status[provider_name]["available"] = False
                            self.logger.warning(
                                f"Provider {provider_name} marked as unavailable due to health check failure"
                            )
    ...
    def get_best_provider(self) -> Optional[LLMProviderInterface]:
        ...
        for provider in self.providers:
            ...
            if self.provider_status[provider_name][
                "available"
            ] and not self._is_circuit_breaker_open(provider_name):
