...
class HuggingFaceLLMProvider(LLMProviderInterface):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.use_api:
            ...
            if not self.api_key:
                raise LLMConnectionError("HuggingFace API key is required when use_api=True")
    ...
    def _generate_with_local(self, prompt: str, **kwargs) -> str:
        ...
        # Generate
        try:
            outputs = self.generation_pipeline(prompt, **generation_kwargs)
...
{raw_text}"""
    ...
    async def detect_relationships(
        ...
        try:
            ...
            response = await self.generate_completion(
                prompt, LLMTask.RELATIONSHIP_DETECTION, temperature=0.1
            )
...
Only include relationships with high confidence (>0.7)."""
    ...
    async def parse_natural_language_query(
        ...
        try:
            ...
            response = await self.generate_completion(
                prompt, LLMTask.NATURAL_LANGUAGE_QUERY, temperature=0.1
            )
...
- Be conservative with confidence scores"""
    ...
    async def validate_content(self, content: str, criteria: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.generate_completion(
                prompt, LLMTask.CONTENT_VALIDATION, temperature=0.1
            )
