...
class PerformanceMonitor:
    ...
    def __init__(
        self, alert_thresholds: Optional[Dict[str, float]] = None, monitoring_interval: float = 30.0
    ):
        """
    ...
    def record_ingestion_performance(
        ...
        throughput = (
            (items_processed / (processing_time_ms / 1000.0)) if processing_time_ms > 0 else 0
        )
    ...
    def _collect_resource_metrics(self) -> ResourceMetrics:
        ...
        network_sent_delta = (network_io.bytes_sent - self._last_network_io.bytes_sent) / (
            1024 * 1024
        )
        network_recv_delta = (network_io.bytes_recv - self._last_network_io.bytes_recv) / (
            1024 * 1024
        )
    ...
    def _check_query_alerts(self, metric: QueryMetrics):
        ...
        if (
            metric.execution_time_ms > self.alert_thresholds.get("query_avg_time_ms", float("inf"))
            and not metric.error
        ):
            self._trigger_alert(
                "query_performance",
                "warning",
    ...
    def _trigger_alert(
        self, metric_type: str, severity: str, message: str, value: float, threshold: float
    ):
    ...
    def get_performance_recommendations(self) -> List[str]:
        ...
        if query_stats:
            ...
            if query_stats.get("cache_hit_rate", 0) < 0.3:
                recommendations.append(
                    "Consider optimizing query caching - low cache hit rate detected"
                )

            if query_stats.get("average_execution_time_ms", 0) > 1000:
                recommendations.append(
                    "Query execution times are high - consider optimizing query patterns or adding indexes"
                )

            if query_stats.get("error_rate", 0) > 0.01:
                recommendations.append(
                    "Query error rate is elevated - review query validation and error handling"
                )
        ...
        if resource_stats:
            ...
            if cpu_avg > 70:
                recommendations.append(
                    "High CPU utilization - consider scaling up or optimizing algorithms"
                )

            memory_avg = resource_stats.get("memory_utilization", {}).get("average_percent", 0)
            if memory_avg > 80:
                recommendations.append(
                    "High memory utilization - consider increasing memory or optimizing data structures"
                )
