...
class PrometheusMetricsCollector:
    ...
    def __init__(
        self, performance_monitor: PerformanceMonitor, registry: Optional[CollectorRegistry] = None
    ):
        """
    ...
    def _init_metrics(self):
        ...
        self.system_memory_used_bytes = Gauge(
            "memory_engine_memory_used_bytes", "Memory used in bytes", registry=self.registry
        )
        ...
        self.cache_hit_rate_gauge = Gauge(
            "memory_engine_cache_hit_rate", "Query cache hit rate (0-1)", registry=self.registry
        )
    ...
    def _handle_alert(self, alert: PerformanceAlert):
        ...
        with self._lock:
            ...
            self.logger.debug(
                f"Recorded Prometheus alert metric: {alert.metric_type} ({alert.severity})"
            )
    ...
    def update_metrics(self, force: bool = False):
        ...
        with self._lock:
            try:
                ...
                self.monitoring_active_gauge.set(
                    1 if summary.get("monitoring_active", False) else 0
                )
    ...
    def _update_query_metrics(self, query_stats: Dict[str, Any]):
        ...
        for query_type, type_stats in by_type.items():
            ...
            self.query_duration_histogram.labels(
                query_type=query_type, cache_hit="unknown"
    ...
    def _update_ingestion_metrics(self, ingestion_stats: Dict[str, Any]):
        ...
        for op_type, type_stats in by_type.items():
            ...
            self.ingestion_items_counter.labels(
                operation_type=op_type, status="success"
            )._value._value = count  # Set absolute value
    ...
    def record_query_metric(
        ...
        with self._lock:
            # Execution time
            self.query_duration_histogram.labels(
                query_type=query_type, cache_hit=str(cache_hit).lower()
            ).observe(execution_time_ms / 1000.0)

            # Query count
            status = "error" if error else "success"
            self.query_total_counter.labels(query_type=query_type, status=status).inc()

            # Result count
            if not error:
                self.query_result_count_histogram.labels(query_type=query_type).observe(
                    result_count
                )
    ...
    def record_ingestion_metric(
        ...
        with self._lock:
            # Processing time
            self.ingestion_duration_histogram.labels(operation_type=operation_type).observe(
            ...
            if processing_time_ms > 0:
                ...
                self.ingestion_throughput_gauge.labels(operation_type=operation_type).set(
                    throughput
                )
...
class MetricsIntegration:
    ...
    def __init__(
        ...
        self.performance_monitor = PerformanceMonitor(
            alert_thresholds=alert_thresholds, monitoring_interval=monitoring_interval
        )
    ...
    def record_ingestion_performance(
        self,
        operation_id: str,
        operation_type: str,
        items_processed: int,
