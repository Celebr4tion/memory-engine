...
def update_rating(node_id: str, evidence: Dict[str, float], storage=None) -> Dict[str, Any]:
    ...
    try:
        ...
        if "confirmation" in evidence or "contradiction" in evidence:
            ...
            new_truthfulness = min(
                1.0, max(0.0, old_truthfulness + 0.2 * confirmation - 0.2 * contradiction)
            )
        ...
        if "richness" in evidence:
            ...
            new_richness = min(1.0, max(0.0, old_richness + 0.2 * richness_factor))
        ...
        if "stability" in evidence:
            ...
            new_stability = min(1.0, max(0.0, old_stability + 0.2 * stability_factor))
        ...
        if updated_data:
            ...
            old_data = {k: node_data[k] for k in updated_data.keys() if k in node_data}
            ...
            return {"status": "no_changes", "node_id": node_id}
...
class RatingUpdater:
    ...
    def update_all_ratings(
        self,
        node_id: str,
        evidence_dict: Dict[str, float] = None,
        truthfulness_change: float = None,
        richness_change: float = None,
        stability_change: float = None,
    ) -> Dict[str, Any]:
        """
