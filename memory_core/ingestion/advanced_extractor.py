...
def process_extracted_units(
    units: List[Dict[str, Any]], source_label: str, storage=None, embedding_manager=None
) -> List[str]:
    """
        ...
        try:
            ...
            if "metadata" in unit and isinstance(unit["metadata"], dict):
                ...
                if "confidence_level" in unit["metadata"]:
                    try:
                        node_data["rating_truthfulness"] = float(
                            unit["metadata"]["confidence_level"]
                        )
                    except (ValueError, TypeError):
                        pass
