...
class BulkIngestionProcessor:
    ...
    def __init__(self, 
        ...
        self.logger.info(f"Bulk processor initialized: workers={max_workers}, batch_size={batch_size}")
    ...
    def _process_single_document(self, document: BulkDocument) -> int:
        ...
        try:
            ...
            if self.storage or self.embedding_manager:
                ...
                self.logger.debug(f"Document {document.id}: {len(knowledge_units)} units -> {len(node_ids)} nodes")
            else:
                self.logger.debug(f"Document {document.id}: {len(knowledge_units)} units extracted (not stored)")
    ...
    def process_documents_with_embeddings_batch(self, documents: List[BulkDocument]) -> BulkIngestionMetrics:
        """
        ...
        Returns:
            Processing metrics
        """
        self.logger.info(f"Starting bulk processing with embedding batching for {len(documents)} documents")
    ...
    async def process_documents_async(self, documents: List[BulkDocument]) -> BulkIngestionMetrics:
        ...
        self.logger.info(f"Async processing complete: {self.metrics.throughput_docs_per_second:.2f} docs/s")
        return self.metrics
    ...
    def get_performance_recommendations(self) -> Dict[str, str]:
        ...
        if self.metrics.throughput_docs_per_second > 0:
            if self.metrics.throughput_docs_per_second < 1.0:
                recommendations['throughput'] = "Consider increasing batch_size or max_workers for better throughput"
            
            if self.metrics.failed_documents / self.metrics.total_documents > 0.1:
                recommendations['reliability'] = "High failure rate detected - check document quality and error handling"
            
            if self.embedding_manager:
                cache_stats = self.embedding_manager.get_cache_statistics()
                if cache_stats['hit_rate'] < 0.3:
                    recommendations['caching'] = "Low embedding cache hit rate - consider preprocessing similar content"
