...
class HealthCheck:
    ...
    def __init__(
        self, name: str, timeout: float = 10.0, critical: bool = True, interval: float = 30.0
    ):
        self.name = name
    ...
    async def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            is_healthy = await asyncio.wait_for(self._check_health(), timeout=self.timeout)
            ...
            if is_healthy:
                ...
                if self.consecutive_failures >= self.max_failures:
                    ...
                    message = (
                        f"{self.name} has failed {self.consecutive_failures} consecutive checks"
                    )
                else:
    ...
    async def _check_health(self) -> bool:
        ...
        try:
            ...
            if hasattr(self.llm_provider, "generate_completion"):
                response = await self.llm_provider.generate_completion("test", max_tokens=1)
