...
class ServiceMonitor:
    ...
    async def check_http_service(self, name: str, url: str, 
                                ...
                                health_path: str = None) -> ServiceStatus:
    ...
    async def check_ollama_service(self, name: str, base_url: str = "http://localhost:11434") -> ServiceStatus:
    ...
    async def check_api_service(self, name: str, api_key: str, 
                               base_url: str, test_endpoint: str = None) -> ServiceStatus:
        ...
        try:
            ...
            async with session.get(url, headers=headers) as response:
                ...
                if response.status in [200, 401]:  # 401 means service is up but auth failed
                    health = ServiceHealth.HEALTHY if response.status == 200 else ServiceHealth.DEGRADED
                    message = "API service accessible" if response.status == 200 else "API service accessible but authentication may be invalid"
    ...
    async def check_all_services(self, service_configs: List[Dict[str, Any]]) -> Dict[str, ServiceStatus]:
        ...
        for config in service_configs:
            ...
            if service_type == 'http':
                ...
                task = self.check_milvus_service(
                    name,
                    config['host'],
                    config.get('port', 19530)
                )
    ...
    async def get_system_dependencies_status(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if storage_config.get('backend') == 'janusgraph':
            service_configs.append({
                'name': 'janusgraph',
                'type': 'janusgraph',
                'url': storage_config.get('janusgraph', {}).get('url', 'ws://localhost:8182/gremlin')
            })
        ...
        healthy_count = sum(1 for status in service_results.values() 
